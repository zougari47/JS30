...
const radomSortData = randomSort(data);
const cards_html = radomSortData
  .map((obj) => {
    return `
    <div class="col-sm-12 col-md-4 col-lg-3 mb-5 ">
        <div data-url="${obj.link}" class="card h-100 position-relative">
            <img
                src="${obj.img}"
                class="card-img-top"
                alt=""
            />
            <div class="txt position-relative container-fluid d-flex justify-content-center align-items-center ">
              <h3 class="title mx-auto mt-3 center-block p-3">${obj.title}</h3>
            </div>
...
cards.forEach((card) => card.addEventListener('click', openLink));
